...
export default function BottomStatus() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-10 bg-black bg-opacity-20 border border-slate-50 text-slate-500 p-2 font-mono text-xs sm:text-sm">
...
function Popup({ content, onClose }: { content: PopupContent, onClose: () => void }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-black bg-opacity-50 border border-slate-500 rounded p-3 shadow-lg"
    >
