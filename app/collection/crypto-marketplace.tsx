...
export default function MainComponent() {
  ...
  return (
    <div className="min-h-screen bg-black text-gray-300 p-4">
      <div className="max-w-full mx-auto">
        <motion.div
          layout
          className="bg-black rounded-lg overflow-auto  md:overflow-hidden"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
  ...
  {tabs.map((tab) => (
    <motion.button
      key={tab.id}
      className={`group flex items-center px-4 py-6 text-xl font-medium transition-colors duration-200 ${
        activeTab === tab.id 
          ? 'text-[#F7F2DA] border-b-2 border-[#F7F2DA]' 
          : 'text-slate-500 hover:text-[#F7F2DA]'
      }`}
