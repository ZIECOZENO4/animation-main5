...
const blockchains = [
  { 
    name: "Arbitrum", 
    chainId: 421614,
    color: "bg-gradient-to-r from-indigo-600 to-blue-500 border border-blue-600", 
    logo: "/images/arbi.png", 
    recommended: true 
  },
  { 
    name: "Ethereum", 
    chainId: 11155111,
    color: "bg-gradient-to-r from-indigo-500 to-neutral-700 border border-green-600", 
    logo: "/images/eth.png",
    disabled: true
  },
  { 
    name: "Optimism", 
    chainId: 11155420,
    color: "bg-gradient-to-r from-amber-500 to-pink-500 border border-red-800", 
    logo: "/images/opti.png",
    disabled: true
  },
]
...
export default function Component() {
  ...
  const handleChainSelect = (blockchain: typeof blockchains[0]) => {
    if (blockchain.disabled) return
    ...
    if (blockchain.chainId !== currentChainId) {
      openChainModal?.()
      return
    }

    router.push(`/deploy?chain=${blockchain.chainId}`)
  }
    ...
    <div className="min-h-screen text-[#F7F2DA] p-4 sm:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center sm:items-stretch space-y-6 sm:space-y-0 sm:space-x-4 md:space-x-6 lg:space-x-8 max-w-7xl min-h-screen md:h-[100vh] mx-auto">
        {blockchains.map((blockchain) => (
          <div 
            key={blockchain.name} 
            className={`w-full sm:w-1/3 px-4 md:px-10 md:max-w-sm ${blockchain.disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
            onClick={() => handleChainSelect(blockchain)}
          >
            <TiltCard blockchain={blockchain} />
          </div>
        ))}
...
function TiltCard({ blockchain }: { blockchain: typeof blockchains[0] }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: blockchain.disabled ? 1 : 1.1 }}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      onHoverStart={() => !blockchain.disabled && setIsHovered(true)}
      transition={{ type: "spring", stiffness: 300, damping: 10 }}
    >
      {blockchain.recommended && (
        <div className="absolute top-2 right-2 bg-red-500 text-[#F7F2DA] text-xs sm:text-sm rounded-full px-2 py-1 flex items-center">
